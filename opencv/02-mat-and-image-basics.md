...
**【标注：为什么是 `(x, y)` 但却用 `img[y, x]`？】**

- **几何/坐标的约定**：二维坐标通常写作 `(x, y)`，其中 `x` 表示水平（向右），`y` 表示垂直（向下）
- **数组/矩阵的约定**：图像在代码里通常是二维数组，索引写作 `[row, col]`（先行后列）
- **对应关系**：`row = y`，`col = x`，所以 **`img[y, x]`** 等价于 **`img[row, col]`**

一句话记：**坐标是 (列, 行)，索引是 [行, 列]**。
